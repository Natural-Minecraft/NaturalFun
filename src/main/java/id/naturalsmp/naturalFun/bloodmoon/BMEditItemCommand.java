...
public class BMEditItemCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label,
        ...
        if (!player.hasPermission("bloodmoon.admin")) {
            player.sendMessage(ChatUtils.toComponent("<red>No permission."));
            return true;
        }

        ItemStack item = player.getInventory().getItemInMainHand();
        if (item.getType().isAir()) {
            player.sendMessage(ChatUtils.toComponent("<red>You must hold an item."));
            return true;
        }

        if (args.length < 1) {
            player.sendMessage(ChatUtils.toComponent("<red>Usage: /" + label + " <value>"));
            return true;
        }
        ...
        if (action.equals("bmsetprice")) {
            try {
                ...
                player.sendMessage(ChatUtils.toComponent("<green>Price set to " + price));
            ...
            try {
                ...
                player.sendMessage(ChatUtils.toComponent("<green>Stock set to " + stock));
            } catch (NumberFormatException e) {
                player.sendMessage(ChatUtils.toComponent("<red>Invalid number."));
            }
            ...
            player.sendMessage(ChatUtils.toComponent("<green>Rarity set to " + rarity));
        }
